...
def run(putpol,callpol,w0='0000',w1='9999'):
    ...
    fin=bars[max(x for x in days if x<=w1)] if w1!="9999" else final
    total=shares*fin+prem-paid+recv+divs
...
WINDOWS=[("FULL  2024-07 -> 2026-08  (91.6 -> 82.2, down)","0000","9999"),
         ("RALLY 2025-06 -> 2026-02  (85.2 -> 90.8, up)","2025-06-01","2026-02-27"),
         ("FALL  2026-03 -> 2026-08  (89.6 -> 82.2, down)","2026-03-01","9999")]
for title,w0,w1 in WINDOWS:
    print("\n"+"="*86); print(title); print("="*86)
    hdr="%-13s %8s %7s %9s %9s %9s %11s"%("arm","shares","kept%","premium","called","divs","TOTAL")
    print(hdr); print("-"*len(hdr))
    base={}
    for pp in ("OTM","ATM","ITM"):
        for cp in ("none","far","atm"):
            r=run(pp,cp,w0,w1)
            kept=100*r["shares"]/r["bought"] if r["bought"] else 0
            print("%-13s %8.0f %6.0f%% %9.0f %9.0f %9.0f %11.0f"%(f"{pp} / {cp}",r["shares"],kept,r["prem"],r["recv"],r["divs"],r["total"]))
    print()
