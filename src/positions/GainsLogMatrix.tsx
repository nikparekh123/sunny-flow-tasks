...
const LS_GAINS_WEEKS = 'np:gainsWeeks';
...
export function GainsLogMatrix({ rows, gainsByTicker, onCellClick, onTickerClick }: Props) {
  ...
  const [weeks, setWeeksState] = useState<Weeks>(() => readWeeksLS(LS_GAINS_WEEKS));
  const setWeeks = (w: Weeks) => {
    setWeeksState(w);
    try { window.localStorage.setItem(LS_GAINS_WEEKS, String(w)); } catch { /* private mode */ }
  };
