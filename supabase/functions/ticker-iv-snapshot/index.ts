...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await admin.from('sync_heartbeat').upsert({
      feed: 'ticker-iv-snapshot', ran_at: new Date().toISOString(),
      rows_written: ok_count,
      detail: `snapshot ${snapshotDate} · ${ok_count} of ${heldTickers.length} captured`,
    }, { onConflict: 'feed' });
